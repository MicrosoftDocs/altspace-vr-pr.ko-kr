...
1. **멀티미디어 콘솔 > SDK 앱 > 세계 편집기 > 편집기 패널** 을 선택합니다. (세계 **편집기 > 기본 사항 > SDK 앱으로** 이동하지 마세요. 등록되지 않은 앱용입니다.)  
2. 멀티미디어 콘솔을 배치하여 공간과 대상을 가장 잘 배치합니다.
3. 주황색 편집 모드 단추를 클릭하여 편집 모드에서 나가세요.
4. **미디어 플레이어 소유자인가요?라는** 메시지가 표시됩니다. 이 멀티미디어 콘솔 세션의 공식 소유자여야 하는 사용자인 경우 확인하고 계속합니다. (다른 권한 있는 역할도 사용할 수 있습니다. 자세한 목록은 아래를 참조하세요.)
5. 예를 선택하여 기본 호스트임을 확인합니다.  
6. 웹 포털 또는 유효한 JSON에서 코드를 입력하라는 대화 상자가 팝업됩니다.  대시를 포함하여 웹 포털에서 두 단어 코드를 입력하고 확인을 누릅니다. (JSON은 아래에 설명된 고급 구성입니다.)
...
비디오는 Twitch 및 DLive를 통해 비디오 또는 라이브 스트림을 호스트할 수 있습니다.  (다른 지원은 적절한 스트림 URL을 얻기 위해 추가 작업과 함께 작동할 수 있지만 멀티미디어 콘솔 내에서 완전히 지원되지는 않습니다.)
...
> [!NOTE]
> 필수: 비디오를 제대로 전달할 수 있도록 비디오 길이와 일치하는 시간을 입력합니다. 예를 들어 비디오가 5분 길이가 300초인 경우, 그렇지 않으면 비디오가 다음 콘텐츠로 건너뛰지 않습니다.
...
미디어 목록은 역할 및 재생 설정 같은 JSON 구조의 루트에 설정된 속성입니다.  다음 미디어 구성 구조 중 하나를 포함할 수 있는 간단한 배열입니다. (각각이 수행하는 일에 대한 자세한 내용은 위의 속성 설명을 참조하세요.)
