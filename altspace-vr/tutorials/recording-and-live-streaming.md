...
2. **Windows** 선택하여 OBS를 다운로드합니다. 이 게시물은 **OBS v22.0.2를** 사용하고 있습니다.
3. OBS 설치

### <a name="have-altspacevr-running-in-2d-mode-before-you-run-obs"></a>OBS를 실행하기 전에 AltspaceVR을 2D 모드로 실행해야 합니다.
...
이렇게 하면 AltspaceVR이 OBS에 표시됩니다. 이제 OBS에서 다음 속성을 설정하려면 **파일 > 설정** 로 이동합니다.
...
* AltspaceVR의 오디오만 기록하려면 볼륨 Mixer(Windows 오른쪽 아래에 있는 스피커 아이콘을 마우스 오른쪽 단추로 클릭)로 이동하여 시스템 소리, 브라우저 등을 음소거하지만 OBS 또는 AltspaceVR을 음소거하지 않습니다.

> [!IMPORTANT]
> 기록 후 이러한 볼륨 Mixer 설정을 다시 켜야 합니다.

이제 OBS로 다시 이동하고 파일에서 **기록 중지**>**기록 표시를** 선택합니다. 그러면 OBS 비디오 파일이 있는 폴더가 열리고 테스트 비디오를 두 번 클릭합니다.

경우에 따라 녹음이 매우 시끄러워 데스크톱 오디오에 대한 슬라이더를 낮추고 테스트할 다른 녹음을 만듭니다.
...
AltspaceVR 및 OBS를 설치합니다. 2D 모드에서 AltspaceVR을 시작하고, 라이브 스트림을 시작하거나, YouTube에서 '새 라이브 이벤트'를 만들고, YouTube 스트림 키로 OBS를 설정하고, OBS에서 스트리밍을 시작하고, YouTube에서 스트리밍을 시작하고, 경합을 시작합니다!

### <a name="the-slightly-longer-version"></a>약간 더 긴 버전

1. [https://obsproject.com/](https://obsproject.com/)로 이동합니다.
2. **Windows** 선택하여 OBS를 다운로드합니다(이 게시물은 OBS v22.0.2 사용).
3. OBS 설치

OBS를 실행하기 전에 AltspaceVR을 2D 모드로 실행해야 합니다.
1. 웹 사이트에서 AltspaceVR을 다운로드합니다. [https://account.altvr.com/downloads](https://account.altvr.com/downloads)
2. 2D 모드에서 AltspaceVR을 시작하려면 PC에서 HMD의 USB 케이블을 분리하거나, Ctrl+Alt+Del, 서비스, Oculus VR 런타임 서비스가 있는 경우 마우스 오른쪽 단추로 클릭하고 중지합니다. 이렇게 하면 Oculus Home을 사용하지 않도록 설정하고 2D 모드에서 AltspaceVR을 시작하고, 이러한 단계를 반복하고, VR 모드를 다시 시작합니다.
...
9. 커서 캡처 Scroll down: 확인 선택 취소

이렇게 하면 AltspaceVR이 OBS에 표시됩니다. 잘하셨습니다!

이제 OBS에서 파일>설정 이동합니다.

| 탭 | 설정 |
|---|---|
| 일반 | 스트리밍 시 자동으로 기록 틱(라이브 스트리밍 외에도 비디오 파일을 컴퓨터에 기록함) |
| 스트림 | 스트림 유형: Streaming Services <br> 서비스: YouTube/YouTube 게임(Twitch, Mixer, Facebook Live 등으로 스트리밍할 수도 있습니다.)<br>서버: 기본 YouTube ingest server <br>스트림 키: YouTube에서 스트림 키 붙여넣기(아래 'YouTube에서 라이브 스트리밍 설정' 참조) |
| 출력 | 출력 모드: 고급으로 전환 |
...
AltspaceVR의 오디오만 기록하려면 볼륨 Mixer(Windows 오른쪽 아래에 있는 스피커 아이콘을 마우스 오른쪽 단추로 클릭)로 이동하고 시스템 소리, 브라우저 등을 음소거하지만 OBS 또는 AltspaceVR을 음소거하지 않습니다.

;) 녹음한 후 오디오를 다시 켜야 합니다.
...
* Stream 설정 기본값을 유지합니다.<br>
* 스트림 키(인코더에 붙여넣기) '표시'를 선택하고 이 키를 복사하여 OBS에 붙여넣을 수 있습니다.<br>
* OBS/설정/Stream 열기<br>
...
* 제목, 날짜, 시작 시간, 설명, 업로드 미리 보기 및 태그 추가 – AltspaceVR :) 태그를 잊지 마세요.
